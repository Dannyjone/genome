...
/// <returns type="Array" />
var getProcessPoints = function (points, tolerance) {
    if (!points.length) { //当points不是数组或没有值时，直接返回一个空数组
        return [];
    }
...
var reduce = function (points, firstPoint, lastPoint, tolerance, pointIndexsToKeep) {
    var maxDis = 0,
...
//Height = Area/.5/Base
var perpendicularDistance = function (beginPoint, endPoint, comparePoint) {
    var area = Math.abs(0.5 * (
          beginPoint.TrackStart * endPoint.TrackEnd 
        + endPoint.TrackStart * comparePoint.TrackEnd 
        + comparePoint.TrackStart * beginPoint.TrackEnd 
        - endPoint.TrackStart * beginPoint.TrackEnd 
        - comparePoint.TrackStart * endPoint.TrackEnd 
        - beginPoint.TrackStart * comparePoint.TrackEnd));
    var bottom = Math.sqrt(Math.pow(beginPoint.TrackStart - endPoint.TrackStart, 2) 
               + Math.pow(beginPoint.TrackEnd - endPoint.TrackEnd, 2));
    var height = area / bottom * 2;
    return height;
}

exports.GetProcessPoints = getProcessPoints